...
operator/(const complex<_Tp>& __z, const complex<_Tp>& __w)
{
    ...
    if (isnan(__x) && isnan(__y))
    {
        ...
        else if ((std::isinf(__a) || std::isinf(__b)) && isfinite(__c) && isfinite(__d))
        {
            __a = copysign(std::isinf(__a) ? _Tp(1) : _Tp(0), __a);
            __b = copysign(std::isinf(__b) ? _Tp(1) : _Tp(0), __b);
            __x = _Tp(INFINITY) * (__a * __c + __b * __d);
...
#pragma warning(suppress: 4756) // Ignore INFINITY related warning
        ...
        {
            __a = copysignf(std::isinf(__a) ? float(1) : float(0), __a);
            __b = copysignf(std::isinf(__b) ? float(1) : float(0), __b);
#pragma warning(suppress: 4756) // Ignore INFINITY related warning
...
#pragma warning(suppress: 4756) // Ignore INFINITY related warning
        ...
        {
            __c = copysignf(std::isinf(__c) ? float(1) : float(0), __c);
            __d = copysignf(std::isinf(__d) ? float(1) : float(0), __d);
