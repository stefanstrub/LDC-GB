...
for ind in range(4): #[3,8,9]
    ...
    ax1.loglog(f[peaks[indexes[-2]]]*1000, np.sqrt(psdX[peaks[indexes[-2]]]),'.', label="most prominent peak")
    ...
    plt.legend()
    plt.show()
    selected_frequency = f[peaks[indexes[-2]]]
    amplitude = np.sqrt(psdX[peaks[indexes[-2]]])
