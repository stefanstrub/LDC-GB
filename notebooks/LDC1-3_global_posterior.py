...
for n in range(3):
    ...
    for i in range(len(found_sources_mp)):
        for j in range(len(found_sources_in[i])):
            ...
            df = pd.read_csv(grandparent+"/LDC/pictures/LDC1-3_v2"+'/Chain/frequency'+str(int(np.round(save_frequency*10**9)))+'nHz'+save_name+'.csv')
            if parameter1 == 'Inclination':
...
for n, ax in enumerate(g.subplots[:,0]):
    ...
    #     ax.set_yscale('log')
g.export('/home/stefan/LDC/pictures/global fit all '+save_name+'log frequencyfastGB.png')