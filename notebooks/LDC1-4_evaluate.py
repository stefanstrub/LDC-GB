...
#         list_SNR = []
#         for k in range(3):
#             list_SNR.append(search1.loglikelihood_SNR([found_sources_mp_all[i][j+k]]))
...
# np.save(SAVEPATH+'/found_sources_pGB_injected_array2_half_year'+ str(int(np.round(search_range[0]*10**8)))+'to'+ str(int(np.round(search_range[1]*10**8))) +save_name+'.npy', np.asarray(pGB_injected))
pGB_injected = np.load(SAVEPATH+'/found_sources_pGB_injected_array2_half_year'+ str(int(np.round(search_range[0]*10**8)))+'to'+ str(int(np.round(search_range[1]*10**8))) +save_name+'.npy', allow_pickle = True)
...
print('matched signals/found signals:', number_of_matched_signals/number_of_found_signals)
...
for i in range(len(found_sources_matched)):
    ...
    for j in range(len(found_sources_matched[i])):
        ...
        found_sources_matched[i][j]['SNR'] = search1.SNR([pGB_dict])
...
for i in range(len(frequencies_search)):
    ...
    if len(pGB_injected[i]) > 0:
        if len(pGB_injected[i]) > 20:
            search1.plot(found_sources_in=found_extended, pGB_injected= injected_extended[:20], pGB_injected_matched= matched_extended, saving_label =SAVEPATH+'/strain added'+ str(int(np.round(lower_frequency*10**8))) +save_name+'.png') 
        else:
...
for i in range(len(found_sources_matched)):
    for j in range(len(found_sources_matched[i])):
        if not(is_labeled):
            plt.scatter(found_sources_matched[i][j]['Frequency']*10**3,found_sources_matched[i][j][parameter_to_plot], color = 'orange', label = 'Match')
            is_labeled = True
        else:
            plt.scatter(found_sources_matched[i][j]['Frequency']*10**3,found_sources_matched[i][j][parameter_to_plot], color = 'orange')
is_labeled = False
for i in range(len(found_sources_not_matched)):
    for j in range(len(found_sources_not_matched[i])):
        if not(is_labeled):
            plt.scatter(found_sources_not_matched[i][j]['Frequency']*10**3,found_sources_not_matched[i][j][parameter_to_plot], s=80, facecolors = 'none', edgecolors = 'blue', label = 'found not matched')
            is_labeled = True
        else:
            plt.scatter(found_sources_not_matched[i][j]['Frequency']*10**3,found_sources_not_matched[i][j][parameter_to_plot], s=80, facecolors = 'none', edgecolors = 'blue')
# is_labeled = False
...
#     for j in range(len(pGB_injected_not_matched[i])):
#         if not(is_labeled):
#             plt.scatter(pGB_injected_not_matched[i][j]['Frequency']*10**3,pGB_injected_not_matched[i][j]['IntrinsicSNR'],marker='x', color = 'red', label = 'injected not matched')
#             is_labeled = True
#         else:
#             plt.scatter(pGB_injected_not_matched[i][j]['Frequency']*10**3,pGB_injected_not_matched[i][j]['IntrinsicSNR'],marker='x', color = 'red')
#         if j > 3:
#             break
plt.yscale('log')
plt.xscale('log')
...
plt.legend()
# plt.savefig(SAVEPATH+'/Evaluation/'+parameter_to_plot,dpi=300,bbox_inches='tight')
plt.show()
...
plt.ylabel('Count')
plt.yscale('log')
# plt.savefig(SAVEPATH+'/Evaluation/correlation',dpi=300,bbox_inches='tight')
plt.show()
...
for i in range(len(found_sources_in)):
    ...
    for j in range(len( pGB_injected[i][:10])):
        ...
        print('true subtracted',np.round(search_subtracted.SNRm([pGB_injected[i][j]])[2].values,2), 'original data', np.round(search1.SNRm([pGB_injected[i][j]])[2].values,2))
    ...
    for j in range(len(found_sources_in[i])):
        ...
        for n in range(len( found_sources_in[i][:j])):
            ...
            for k in ["X", "Y", "Z"]:
                tdi_fs_subtracted[k].data[index_low:index_high] = tdi_fs_subtracted[k].data[index_low:index_high] - source_subtracted[k].data
        search_subtracted = Search(tdi_fs_subtracted,Tobs, frequencies_search[i][0], frequencies_search[i][1])
        print('found subtracted',np.round(search_subtracted.SNRm([found_sources_in[i][j]])[2].values,2), 'original data', np.round(search1.SNRm([found_sources_in[i][j]])[2].values,2))
