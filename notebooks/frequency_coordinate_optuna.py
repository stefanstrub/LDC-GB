...
'EclipticLongitude': [0.0, 2.0*np.pi],'Frequency': [pGB['Frequency']*0.9995,pGB['Frequency']*1.0005],'FrequencyDerivative': [10**-20.0, 10**-16.0],
...
for n in range(5):
    ...
    previous_best = loglikelihood(minpGB)
    ...
    # minpGB = scaletooriginal(previous_min,boundaries)
...
# if 'Frequency' in [parameter,parameter2]:
#     resolution_reduced = int(15**2)
train_samples = sampler(resolution_reduced,parameters,minpGB,boundaries,p1, uniform= False, twoD = False, onlyparameter=parameter, secondparameter=parameter2)
train_x = np.zeros((resolution_reduced,len(parameters)))
i = 0
for name in parametersfd:
    train_x[:,i] = train_samples[name]
...
min_parameters = flatsamplesparameters[minindx[0]][minindx[1]]
min_loglike = flatsamples.min()
minpGBpredicted = scaletooriginal(min_parameters,boundaries)
if loglikelihood(minpGBpredicted) < loglikelihood(minpGB):
    minpGB = minpGBpredicted 
print('pred',min_loglike,'true',loglikelihood(scaletooriginal(min_parameters,boundaries)),'min', loglikelihood(minpGB), minpGB)
...
ax1.plot(Xs.f*1000, Xs.values.real, label='true response')
ax2=plt.subplot(232)
...
Ys = Ys[index_low:index_low+len(dataY)]
Zs = Zs[index_low:index_low+len(dataZ)]
ax1.plot(Xs.f*1000, Xs, label='found response')
