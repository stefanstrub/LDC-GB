...
def loglikelihood(pGBs):
    for i in range(len(pGBs)):
        ...
        if i == 0:
            ...
            Ys_total += Ys[index_low : index_low + len(dataY)]
            Zs_total += Zs[index_low : index_low + len(dataZ)]
        if len(Xs_total) != len(dataX):
            Xs_total = deepcopy(dataX)
            Xs_total[0:len(Xs[index_low : index_low + len(dataX)])] = Xs[index_low : index_low + len(dataX)]
            Xs_total[len(Xs[index_low : index_low + len(dataX)]):] = np.zeros(len(Xs_total[len(Xs[index_low : index_low + len(dataX)]):]))
            Ys_total = deepcopy(dataY)
            Ys_total[0:len(Ys[index_low : index_low + len(dataY)])] = Ys[index_low : index_low + len(dataY)]
            Ys_total[len(Ys[index_low : index_low + len(dataY)]):] = np.zeros(len(Ys_total[len(Ys[index_low : index_low + len(dataY)]):]))
            Zs_total = deepcopy(dataZ)
            Zs_total[0:len(Zs[index_low : index_low + len(dataZ)])] = Zs[index_low : index_low + len(dataZ)]
            Zs_total[len(Zs[index_low : index_low + len(dataZ)]):] = np.zeros(len(Zs_total[len(Zs[index_low : index_low + len(dataZ)]):]))
...
def CoordinateMC(n):
    ...
    for i in range(100):
        # if i > 48:
        #     n_trials = 50
        parameter1 = parametersfd[i % 8]
        parameter2 = parametersfd[np.random.randint(0, 7)]
        ...
        study.optimize(lambda trial: objective(trial, changeableparameters, maxpGB2, signal), n_trials=n_trials)
    ...
    parameters_recorded[n] = parameters_recorded1[0]
    pbar.update(1)
...
for ind in range(10): #[3,8,9]
    ...
    # ax1.loglog(f[peaks[indexes[signal_peak]]]*1000, np.sqrt(psd[peaks[indexes[signal_peak]]]),'.', label="most prominent peak")
    ...
    # plt.legend()
    # plt.show()
    selected_frequency = f[peaks[indexes[signal_peak]]]
    amplitude = np.sqrt(psd[peaks[indexes[signal_peak]]])
    ...
    boundaries = {
        ...
        # "FrequencyDerivative": [np.log10(5e-6*pGB['Frequency']**(13/3)),np.log10(8e-8*pGB['Frequency']**(11/3))],
        "Inclination": [-1.0, 1.0],
        "InitialPhase": [0.0, 1.0 * np.pi],
        "Polarization": [0.0, 1.0 * np.pi],
    }
    if boundaries['FrequencyDerivative'][0] > boundaries['FrequencyDerivative'][1]:
        c = boundaries['FrequencyDerivative'][0]
        boundaries['FrequencyDerivative'][0] = boundaries['FrequencyDerivative'][1]
        boundaries['FrequencyDerivative'][1] = c
    ...
    # pGBs = [{'Amplitude': 2.776497384629967e-22, 'EclipticLatitude': -0.37846670464024873, 'EclipticLongitude': 4.468096210465891, 'Frequency': 0.016205266868846793, 'FrequencyDerivative': 8.575033723299429e-14, 'Inclination': 0.4268075606225952, 'InitialPhase': 2.835588082368558, 'Polarization': 2.3419811907883976, 'Loglikelihood': -14.669083613624418}]
    # pGBs = [{'Amplitude': 2.776497384629967e-22, 'EclipticLatitude': -0.37846670464024873, 'EclipticLongitude': 4.468096210465891, 'Frequency': 0.01620542, 'FrequencyDerivative': 8.575033723299429e-14, 'Inclination': 0.4268075606225952, 'InitialPhase': 2.835588082368558, 'Polarization': 2.3419811907883976, 'Loglikelihood': -14.669083613624418}]
    # pGBs = pGBs[0]
    ...
    for parameter in parametersfd + ["Log-likelihood"]:
        ...
        if parameter in ["Log-likelihood"]:
            ...
            ax[j, i % 5].set_xlim(10**(boundaries[parameter][0]), 10**(boundaries[parameter][1]))
        elif parameter == "Inclination":
