...
save_name = 'Sangria_1_full_opt2_cut_all'
...
batch_size = 100
start_index = batch_size*batch_index
# for index_of_interest_to_plot in np.arange(6757,7750):
# for index_of_interest_to_plot in np.arange(6463,6757):
# for index_of_interest_to_plot in np.arange(0,6463):
for index_of_interest_to_plot in np.arange(start_index,start_index+batch_size):
# for index_of_interest_to_plot in np.arange(7618,7750):
    # if index_of_interest_to_plot != 6873:
    #     continue
    print(index_of_interest_to_plot)
    if len(found_sources_mp[index_of_interest_to_plot][3]) == 0:
        print(index_of_interest_to_plot, 'passed')
        continue
    ...
    found_sources_to_subtract = np.concatenate([found_sources_mp[index_of_interest_to_plot-1][3],found_sources_mp[index_of_interest_to_plot+1][3]])
    ...
    if plot_subraction:
        search1.plot(second_data= tdi_fs_subtracted, found_sources_not_matched= found_sources_mp[index_of_interest_to_plot][3])
    # for i in range(len(found_sources_in[index_of_interest_to_plot])):

    loglikelihood_ratio_threshold = 50
    f_transfer = 19.1*10**-3
    if frequencies_search[index_of_interest_to_plot][0] > f_transfer:
        loglikelihood_ratio_threshold = 200

    for i in range(int(len(found_sources_mp_all[index_of_interest_to_plot])/3)):
        ...
        SNR_list = []
        for j in range(3):
            if found_sources_mp_all[index_of_interest_to_plot][i*3+j]['Amplitude'] < 0:
                print('neg amplitude', found_sources_mp_all[index_of_interest_to_plot][i*3+j]['Amplitude'] )
                found_sources_mp_all[index_of_interest_to_plot][i*3+j]['Amplitude'] *= -1
                print('neg amplitude', found_sources_mp_all[index_of_interest_to_plot][i*3+j]['Amplitude'] )
        ...
        found_sources_in_optimize.append(found_sources_mp_all[index_of_interest_to_plot][i*3+max_index]) ## should exclude outsiders
        ...
        if A_optimized.values > 0:
            ...
            print('Amplitude optimization failed with parameters:', found_sources_in_optimize[-1])
            print('switch to optimize with scipy minimize trust-constr')
            found_sources_in_optimize[-1] = search1.optimizeA([[found_sources_in_optimize[-1]]])[0]
        ...
        if search_out_subtracted.loglikelihood([found_sources_in_optimize[-1]]) < loglikelihood_ratio_threshold:
            break
        ...
        start = time.time()
        found_sources_in_optimized_pot = search_out_subtracted.optimize([found_sources_in_optimize], boundaries= total_boundaries)
        print('found optimized log l',search_out_subtracted.loglikelihood(found_sources_in_optimized_pot))
        if search_out_subtracted.loglikelihood(found_sources_in_optimized_pot) > search_out_subtracted.loglikelihood(found_sources_in_optimize):
            found_sources_in_optimize = deepcopy(found_sources_in_optimized_pot)
        print('global optimization time', time.time()-start)

        found_sources = found_sources_in_optimize + found_sources_out_optimize
        # overwrite found_sources_in
        found_sources_mp[index_of_interest_to_plot][3] = found_sources_in_optimize
        ...
        if plot_subraction:
            ...
            # search_out_subtracted.plot(second_data= tdi_fs_subtracted, found_sources_not_matched = found_sources)


np.save(SAVEPATH+'found_signals/found_sources'+ str(int(np.round(search_range[0]*10**8)))+'to'+ str(int(np.round(search_range[1]*10**8))) +save_name+'_full_frequency_cut'+str(batch_index)+'.npy', found_sources_mp)

# found_sources_mp_mid_f_reduced = np.load(SAVEPATH+'found_signals/found_sources'+ str(int(np.round(search_range[0]*10**8)))+'to'+ str(int(np.round(search_range[1]*10**8))) +save_name+'_mid_frequencycut'+batch_index+'.npy', allow_pickle=True)
# found_in_flat_mid_f_reduced = np.concatenate(found_sources_mp_mid_f_reduced[6757:,3])

# found_in_flat = np.concatenate(found_sources_mp[6757:,3])

for batch_index in np.arange(12,77):
    found_sources_mp_new = np.load(SAVEPATH+'found_signals_cut/found_sources30000to9261573Sangria_1_full_opt2_mid_f_cut_full_frequency_cut'+str(batch_index)+'.npy', allow_pickle=True) 
    batch_size = 100
    start_index = batch_size*batch_index
    for index_of_interest_to_plot in np.arange(start_index,start_index+batch_size):
        found_sources_mp[index_of_interest_to_plot][3] = found_sources_mp_new[index_of_interest_to_plot][3]

np.save(SAVEPATH+'found_signals/found_sources'+ str(int(np.round(search_range[0]*10**8)))+'to'+ str(int(np.round(search_range[1]*10**8))) +save_name+'_full_frequency_cut_all.npy', found_sources_mp)
