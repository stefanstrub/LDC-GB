...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            pi_total, _ = self.pi.forward(state)
            action = pi_total.probs.argmax()
            v = self.v.forward(state)
            prob = self.pi._log_prob_from_distribution(pi_total, action)
        return int(action), float(v), float(prob)
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            values = self.ac.v.forward(data['obs'])
            # print(log_prob)
            loss_pi = -(torch.sum(torch.mul(data["ret"],log_prob))/len(ep_returns))
            print(loss_pi)
            # loss_pi = loss_pi/len(ep_returns)
            ...
            values_old = self.ac.v.forward(data['obs']).detach()
            for _ in range(100):
                ...
                values = self.ac.v.forward(data['obs'])
                self.std_ret = data["ret"].std()
                loss_v = criterion(values,data["ret"])
