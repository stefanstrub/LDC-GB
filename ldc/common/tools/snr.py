...
def compute_tdi_snr(source, noise, data=None, fmin=-1, fmax=-1, full_output=False):
    ...
    for k in ["X", "Y", "Z"]:     # individual SNR
        d = data[k].sel(f=slice(fmin, fmax))
        s = source[k].sel(f=slice(fmin, fmax))
    ...
    for k1,k2 in [("X", "Y"), ("X", "Z"), ("Y", "Z")]:
        d1 = data[k1].sel(f=slice(fmin, fmax))
        s1 = source[k1].sel(f=slice(fmin, fmax))
        d2 = data[k2].sel(f=slice(fmin, fmax))
        s2 = source[k2].sel(f=slice(fmin, fmax))
